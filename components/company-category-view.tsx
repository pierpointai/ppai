...
import { ChevronDown, RefreshCw, Building2 } from "lucide-react"
...
interface CompanyCategoryViewProps {
  ...
  vessels: any[]
  isExpanded: boolean
  onToggleExpand: (company: string) => void
...
// Function to get a color for a company based on its name
const getCompanyColor = (company: string) => {
  // Simple hash function to generate a consistent color for each company
...
export function CompanyCategoryView({
  ...
  return (
    <Card key={company} className={cn("overflow-hidden border-2", isMobile && "rounded-xl shadow-sm")}>
      <div
        className={cn(
            ...
            <div className="p-4 bg-white dark:bg-gray-950 border-t">
              {vessels.length > 0 ? (
                <>
                  <div className="flex justify-between items-center mb-3">
                    <h4 className="text-sm font-medium text-gray-700 dark:text-gray-300">
                      {vessels.length} {vessels.length === 1 ? "vessel" : "vessels"} offered by {company}
                    </h4>
                ...
                <div className="text-center py-8 text-muted-foreground">
                  No vessels available from {company}
                  <div className="mt-4">
                    <Button
                      variant="outline"
                      size="sm"
                      onClick={() => {
                        const newOffer = getRandomOfferForCategory("Handysize")

                        // Set the charterer to this company
                        newOffer.charterer = company
                        ...
                        toast({
                          ...
                          description: `Added a vessel from ${company} to your dashboard.`,
                        })
                      }}
                    >
      ...
      {!isExpanded && (
        <div className="p-3 bg-slate-50 dark:bg-slate-900 text-center text-sm text-slate-500 dark:text-slate-400 border-t border-slate-200 dark:border-slate-700">
          {isMobile ? "Tap" : "Click"} to view {vessels.length} {vessels.length === 1 ? "vessel" : "vessels"} from{" "}
          {company}
        </div>
      )}
    </Card>
