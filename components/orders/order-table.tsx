...
export function OrderTable({ orders: propOrders, onAIMatch }: OrderTableProps) {
  ...
  // Add sorting logic before renderOrderTable function
    ...
    return [...orders].sort((a, b) => {
      ...
      switch (sortColumn) {
        case "order":
          ...
          break
        case "type":
          aValue = a.orderType || "Voyage"
          bValue = b.orderType || "Voyage"
          break
        case "cargo":
          aValue = a.cargoType || ""
          bValue = b.cargoType || ""
          break
        case "vesselSize":
          aValue = (a.dwtMin || 0) + (a.dwtMax || 0)
          bValue = (b.dwtMin || 0) + (b.dwtMax || 0)
          break
        case "laycan":
          aValue = new Date(a.laycanStart || "").getTime()
          bValue = new Date(b.laycanStart || "").getTime()
          break
        case "route":
          aValue = `${a.loadPort || ""} ${a.dischargePort || ""}`
          bValue = `${b.loadPort || ""} ${b.dischargePort || ""}`
          break
        case "vessels":
          aValue = (a.linkedVessels || []).length
          bValue = (b.linkedVessels || []).length
          break
        default:
          return 0
      }
        ...
        <TableHeader>
          <TableRow className="bg-muted/50">
            ...
            <TableHead>
              ...
              >
                Cargo
                <SortIndicator column="cargo" />
              </button>
            </TableHead>
            <TableHead>
              <button
                onClick={() => handleSort("vesselSize")}
                className="flex items-center gap-1 hover:text-foreground transition-colors font-medium"
              >
                Vessel Size
                <SortIndicator column="vesselSize" />
              </button>
            </TableHead>
            <TableHead>
              <button
                onClick={() => handleSort("laycan")}
                className="flex items-center gap-1 hover:text-foreground transition-colors font-medium"
              >
                Laycan
                <SortIndicator column="laycan" />
              </button>
            </TableHead>
            <TableHead>
              <button
